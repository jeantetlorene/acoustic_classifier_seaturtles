...
class Evaluation:
    ...
    def _calc_amplitudes_to_predict(self, file_name_no_extension, preprocessing_arg, Saved_amplitudes=True,  verbose = True):
        ...
        audio_amps, original_sample_rate = self.prep.read_audio_file(
            str(
                Path(
                    self.prep.audio_path, file_name_no_extension + self.prep.audio_extension
                )
            )
        )
    ...
    def _repair_svl(
        ...
        for file in file_names.values:
            ...
            audio_amps, original_sample_rate = self.prep.read_audio_file(str(Path(
    ...
    def _testing_dataset_run(self, model, type="test", print_report=True):
        data_path = Path(self.species_folder, "Datasets")
